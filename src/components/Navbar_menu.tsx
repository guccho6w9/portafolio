    ...
    >
      <div className="flex items-center justify-between w-full sm:w-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button className="hover:text-teal-400 transition duration-300">
            EN / ES
          </button>
        </div>
      ...
      <ul
        className={`flex flex-col space-y-4 ${isMenuOpen ? "block" : "hidden"} sm:flex sm:flex-row sm:space-y-0 sm:space-x-6 transition-all duration-500 ease-in-out`}
      >
        {["Experiencia", "Proyectos", "Sobre mí", "Contacto"].map((item) => (
          <li key={item}>
            <button
              onClick={() => scrollToSection(item.toLowerCase())} // Cambiado de <a> a <button> para evitar conflictos
              className="relative group text-xl sm:text-lg"
            >
              <span className="hover:underline hover:text-teal-400 transition duration-300">
                {item}
              </span>
